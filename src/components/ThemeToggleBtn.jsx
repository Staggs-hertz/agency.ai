...
const ThemeToggleBtn = ({theme, setTheme}) => {
  ...
  return (
    <div>
      <button>
        {theme === 'dark' ? (<img src={assets.sun_icon} className='size-8.5 p-1.5 border border-gray-500 rounded-full' alt="sun icon" onClick={() => setTheme('light')} />) : (<img src={assets.moon_icon} className='size-8.5 p-1.5 border border-gray-500 rounded-full' alt="moon icon" onClick={() => setTheme('dark')} />)}
      </button>
    </div>
  )
}
